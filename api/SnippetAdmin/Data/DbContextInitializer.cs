...
namespace SnippetAdmin.Data
{
    public class DbContextInitializer
    {
        private static Action<IMemoryCache, SnippetAdminDbContext, UserManager<RbacUser>,
            RoleManager<RbacRole>, ILogger<SnippetAdminDbContext>, IConfiguration>
            _initialSnippetAdminDbContext = (memoryCache, dbContext, userManager, roleManager, logger, configuration) =>
            {
                ...
                if (dbContext.Database.EnsureCreated())
                {
                    ...
                    // 初始化系统配置
                    logger.LogInformation("初始化系统配置。");
                    InitialSettings(dbContext).Wait();
                    logger.LogInformation("初始化系统配置完成。");
                }
                else
                {
                    // 将所有数据加载到缓存
        ...
        public static Action<IMemoryCache, SnippetAdminDbContext, UserManager<RbacUser>, RoleManager<RbacRole>, ILogger<SnippetAdminDbContext>, IConfiguration> InitialSnippetAdminDbContext
        {
        ...
        {
            dbContext.SysSettingGroups.AddRange(
                new SysSettingGroup { Name = "通用配置1", Code = "Group1", Index = 1 },
                new SysSettingGroup { Name = "通用配置2", Code = "Group2", Index = 3 },
                new SysSettingGroup { Name = "通用配置3", Code = "Group3", Index = 2 },
                new SysSettingGroup { Name = "通用配置4", Code = "Group4", Index = 4 },
                new SysSettingGroup { Name = "通用配置5", Code = "Group5", Index = 5 },
                new SysSettingGroup { Name = "通用配置6", Code = "Group6", Index = 6 });

            dbContext.SysSettingSubGroups.AddRange(
                new SysSettingSubGroup { Name = "应用设置1", Describe = "这是应用设置1", GroupCode = "Group1", Code = "SubGroup1", Index = 1 },
                new SysSettingSubGroup { Name = "应用设置2", Describe = "这是应用设置2", GroupCode = "Group1", Code = "SubGroup2", Index = 2 },
                new SysSettingSubGroup { Name = "应用设置3", Describe = "这是应用设置3", GroupCode = "Group1", Code = "SubGroup3", Index = 3 },
                new SysSettingSubGroup { Name = "应用设置4", Describe = "这是应用设置4", GroupCode = "Group2", Code = "SubGroup4", Index = 1 });

            dbContext.SysSettings.AddRange(
                new SysSetting
                {
                    GroupCode = "Group1",
                    SubGroupCode = "SubGroup1",
                    Name = "指定配置1",
                    Code = "Identity1",
                    Describe = "配置1",
                    InputType = 1,
